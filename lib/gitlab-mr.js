...
function GitlabMR(configuration) {
  ...
  if (configuration.hasOwnProperty("token_filename")) {
    GitlabMR.token = JSON.parse(fs.readFileSync(configuration.token_filename));
  } else {
    GitlabMR.token = JSON.parse(fs.readFileSync("private-token.json"));
  } 
  ...
  function get_mr_for_project(projectID, callback) {
    req({
      url: base_url + '/api/v3/projects/112/merge_requests', 
      qs: {state: "opened"}, 
      method: 'GET', 
      headers: token
